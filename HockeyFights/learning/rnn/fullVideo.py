    ...
    def build(self, rnn_state=100, avg_result=False):
        ...
        with self.graph.as_default():
            ...
            with tf.name_scope("prediction"):
                score = tf.reduce_mean(tf.reshape(dense, shape=(1, -1, self.num_classes)), axis = 1) if avg_result else dense
...
class FullTrainer(baseNetwork.BaseTrainer):
    ...
    def build(self, learning_rate=1e-2):
        with self.model.graph.as_default():
            with tf.name_scope("input"):
                ...
                y = tf.placeholder(tf.int32, (self.model.batch_size, ))
    ...
    def train(self, dataset, epochs, batch_size=None):
        ...
        train_dataset = lambda: dataset.gen_dataset(utils.shuffle(train_names), by_video=True)

        # it doesn't matter to shuffle test dataset because it will not take any impact on result
        test_dataset = lambda: dataset.gen_dataset(test_names, by_video=True)
