...
def build(num_classes = 2, num_steps = 30):
    ...
    with tf.name_scope("cnn"):
        with tf.name_scope("conv1"):
            ...
            #pool1 = tf.nn.max_pool(conv1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding="VALID", name="pool")
        with tf.name_scope("conv2"):
            ...
            #pool2 = tf.nn.max_pool(conv2, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding="VALID", name="pool")
        ...
        with tf.name_scope("dense"):
            ...
            dense_w = tf.Variable(tf.truncated_normal(shape=(size, 100), stddev=0.1), name="w")
            dense_b = tf.Variable(tf.zeros(shape=(100)), name="b")
            dense_out = tf.tanh(tf.matmul(flatten_conv, dense_w) + dense_b, name="dense")

    with tf.name_scope("rnn"):
        size = dense_out.get_shape().as_list()[-1]
        rnn_inputs = tf.reshape(dense_out, (-1, num_steps, size))
        cell = tf.nn.rnn_cell.GRUCell(20)
