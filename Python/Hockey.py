...
source_dir = 'Data'
dataset = ds.HockeyDataset(source_url, source_dir, max_size=10)
...
if type == Model.RNN_FULL:
    model = learning.fullVideo.FullModel(checkpoint_dir='Models/full', seed=100).build(rnn_state=50, avg_result=True)
    if mode == Mode.TRAIN:
        trainer = learning.fullVideo.FullTrainer(model).build(learning_rate=1e-2)
        losses, accuracies = trainer.train(dataset, epochs=5)

if type == Model.RNN_SIMPLE:
    model = learning.simpleVideo.SimpleModel(checkpoint_dir='Models/simple', seed=100).build(rnn_state=50, num_steps=30, avg_result=True)
    if mode == Mode.TRAIN:
        trainer = learning.simpleVideo.SimpleTrainer(model).build(learning_rate=1e-2)
